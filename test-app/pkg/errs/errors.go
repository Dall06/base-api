...
// or temporarily unavailable. Maps to HTTP 503.
...
// PlanLimitExceededError is an error for when a SaaS plan resource limit is reached.
// Returns HTTP 402 Payment Required to indicate upgrade is needed.
func PlanLimitExceededError(resource string, limit int) error {
	return &customError{
		sentinel: ErrPlanLimitExceeded,
		msg:      fmt.Sprintf("Límite alcanzado: máximo %d %s en tu plan. Actualiza tu plan para agregar más.", limit, resource),
	}
}
