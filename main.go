...
//go:embed go.mod go.sum pkg/* opt/* srv/* cmd/* env/* Dockerfile docker-compose.yml .gitignore .env.example README.md
var templateFS embed.FS

const placeholderModule = "base-api"
...
func printUsage() {
	fmt.Println("Uso: base-cli <comando> [argumentos]")
	fmt.Println("Comandos:")
	fmt.Println("  init <nombre-proyecto> --module <path-modulo>   Inicializa un nuevo proyecto a partir de la plantilla")
...
func runInit() {
	...
	if len(os.Args) < 3 {
		fmt.Println("Uso: base-cli init <nombre-proyecto> --module <nombre-modulo>")
		os.Exit(1)
	}
	...
	err := fs.WalkDir(templateFS, ".", func(path string, d fs.DirEntry, err error) error {
		...
		// Omitir el archivo main.go del instalador raíz en el proyecto generado
		if path == "main.go" || path == "." {
			return nil
		}
...
func runAdd() {
	if len(os.Args) < 4 || os.Args[2] != "service" {
		fmt.Println("Uso: base-cli add service <nombre-servicio>")
		os.Exit(1)
	}
	...
	// Verificar que estemos en la raíz de un proyecto hexagonal (buscando srv/)
